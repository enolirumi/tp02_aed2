...
void logWriter()
{
    FILE *log = fopen("892668_binaria.txt", "w");
...
int main(int argc, char **argv)
{
    ...
    #ifdef _WIN32
        meu_strncpy(tmp_path, getenv("TEMP"), meu_strlen(getenv("TEMP")));
    #else
    ...
    if (selecionados->tamanho == 0)
    {
        for (int i = 0; i < colecao->tamanho; i++)
            colecaoPush(selecionados, colecao->restaurantes[i]);
    }

    selectionSort(selecionados);
    ...
    while (scanf(" %[^\n]", nome) == 1)
    {
        if (isFim(nome))
            break;
        Restaurante *r = getRestauranteByNomeBinario(selecionados, nome);
        if (r != NULL)
            printf("SIM\n");
        else
            printf("NAO\n");
