...
void countingSort(ColecaoRestaurante *c)
{
    int maior = c->restaurantes[0]->capacidade;
    int menor = c->restaurantes[0]->capacidade;
    ...
    for (int i = 1; i < c->tamanho; i++)
    {
        ...
        comparacoes++;
        if (c->restaurantes[i]->capacidade < menor)
            menor = c->restaurantes[i]->capacidade;
    }
    ...
    // Conta ocorrências
    for (int i = 0; i < c->tamanho; i++)
        contagem[c->restaurantes[i]->capacidade - menor]++;

    // Transforma em índices acumulados
    for (int i = 1; i < range; i++)
        contagem[i] += contagem[i - 1];

    // Monta saída percorrendo de trás pra frente (garante estabilidade)
    Restaurante **saida = (Restaurante **)malloc(sizeof(Restaurante *) * c->tamanho);
    for (int i = c->tamanho - 1; i >= 0; i--)
    {
        int pos = contagem[c->restaurantes[i]->capacidade - menor] - 1;
        saida[pos] = c->restaurantes[i];
        movimentacoes++;
        contagem[c->restaurantes[i]->capacidade - menor]--;
    }
...
void logWriter() {
    FILE *log = fopen("892668_countingsort.txt", "w");
